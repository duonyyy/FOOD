...
@Injectable()
export class DemoPaymentGuard implements CanActivate {
  constructor(private readonly configService: ConfigService) {}

  canActivate(_context: ExecutionContext): boolean {
    const enabled = this.configService.get<string>('ENABLE_DEMO_PAYMENT') === 'true';
...
export class DemoPaymentController {
  ...
  constructor(
    ...
    if (apiKey && secretKey) {
      ...
      this.logger.warn('Demo payment gateway is not initialized because credentials are missing');
    }
  }
  ...
  async createCheckout(@Body() checkoutData: CreateDemoCheckoutDto) {
