...
import { GeocodingService } from 'src/infra/maps/geocoding.service';
...
export class RestaurantService {
  ...
  private async clearRestaurantCache(restaurantId?: string, ownerId?: string): Promise<void> {
    await Promise.all([
      ...
      restaurantId
        ? this.cacheService.deleteByPattern(`restaurant:${restaurantId}:*`)
        : Promise.resolve(0),
      ownerId
        ? this.cacheService.deleteByPattern(`restaurant:owner:${ownerId}:*`)
        : Promise.resolve(0),
    ]);
  }
  ...
  async requestRestaurantWithFiles(
    ...
    try {
      ...
      if (createRestaurantDto.latitude && createRestaurantDto.longitude) {
        ...
        try {
          ...
          if (coordinates) {
            ...
            this.logger.log(
              `Geocoding successful for address: ${addressData.street}, ${addressData.city}`,
            );
          } else {
            this.logger.warn(
              `Geocoding returned no results for address: ${addressData.street}, ${addressData.city}`,
            );
          }
        } catch (geoError) {
      ...
      try {
        ...
        if (backgroundFile) {
          const bgUpload = await this.minioService.upload(backgroundFile, backgroundFile.originalname, 'restaurant-backgrounds');
          backgroundUrl = this.minioService.getPublicUrl(bgUpload.fileName);
        }

        // Upload certificate image if provided
        if (certificateFile) {
          const certUpload = await this.minioService.upload(certificateFile, certificateFile.originalname, 'restaurant-certificates');
          certificateUrl = this.minioService.getPublicUrl(certUpload.fileName);
        }
      } catch (uploadError) {
  ...
  async updateWithFiles(
    ...
    this.logger.log(
      `[updateWithFiles] Updating restaurant with ID ${id} with data:`,
      updateRestaurantDto,
    );
    ...
    if (
      ...
      this.logger.log(
        `[updateWithFiles] No address fields provided in DTO, skipping address update.`,
      );
    }
    ...
    try {
      ...
      if (avatarFile) {
        const avatarUpload = await this.minioService.upload(avatarFile, avatarFile.originalname, 'restaurant-avatars');
        restaurant.avatar = this.minioService.getPublicUrl(avatarUpload.fileName);
      }
      if (backgroundFile) {
        const bgUpload = await this.minioService.upload(backgroundFile, backgroundFile.originalname, 'restaurant-backgrounds');
        restaurant.backgroundImage = this.minioService.getPublicUrl(bgUpload.fileName);
      }
      if (certificateFile) {
        const certUpload = await this.minioService.upload(certificateFile, certificateFile.originalname, 'restaurant-certificates');
        restaurant.certificateImage = this.minioService.getPublicUrl(certUpload.fileName);
      }
      ...
      if (avatarFile && oldAvatarUrl) {
        await this.minioService.deleteFile(oldAvatarUrl);
      }
      if (backgroundFile && oldBackgroundUrl) {
        await this.minioService.deleteFile(oldBackgroundUrl);
      }
      if (certificateFile && oldCertificateUrl) {
        await this.minioService.deleteFile(oldCertificateUrl);
      }
  ...
  async updateWithAddressAndFiles(
    ...
    try {
      ...
      // Clean up old images if they were replaced
      if (avatarFile && oldAvatarUrl) {
        await this.minioService.deleteFile(oldAvatarUrl).catch((err) => {
          this.logger.warn(`Failed to delete old avatar: ${err.message}`);
        });
      }

      if (backgroundFile && oldBackgroundUrl) {
        await this.minioService.deleteFile(oldBackgroundUrl).catch((err) => {
          this.logger.warn(`Failed to delete old background image: ${err.message}`);
        });
      }

      if (certificateFile && oldCertificateUrl) {
        await this.minioService.deleteFile(oldCertificateUrl).catch((err) => {
  ...
  async findOne(
    ...
    lat?: number,
    lng?: number,
  ): Promise<Restaurant & { distance: number | null; deliveryTime: number | null }> {
    const restaurant = await this.restaurantRepository.findOne({
      where: { id },
      relations: ['foods'],
    });
  ...
  async findByOwnerId(
    ...
    lat?: number,
    lng?: number,
  ): Promise<(Restaurant & { distance: number | null; deliveryTime: number | null }) | null> {
  ...
  async getRevenueByOwner(ownerId: string, month?: string): Promise<number> {
    ...
    const { sum } = await this.foodRepository.manager
      .getRepository('Order')
      ...
      .andWhere(month ? 'order.createdAt >= :startDate AND order.createdAt < :endDate' : '1=1', {
        startDate: month
          ...
          : undefined,
        endDate: month
          ? new Date(Number(month.split('-')[0]), Number(month.split('-')[1]), 1)
          : undefined,
      })
      .getRawOne();
  ...
  async searchRestaurants({
    ...
    return this.cacheService.remember(cacheKey, CACHE_TTL_SECONDS.SHORT, async () => {
      ...
      const [items, initialTotalItems] = await queryBuilder.getManyAndCount();
      let totalItems = initialTotalItems;

      let itemsWithDistance: (Restaurant & {
      ...
      if (lat != null && lng != null) {
        ...
        itemsWithDistance = items.map((r) => ({ ...r, distance: null, deliveryTime: null }));
        itemsWithDistance = itemsWithDistance.slice((page - 1) * pageSize, page * pageSize);
      }
