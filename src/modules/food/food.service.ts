...
import { getDistanceAndDurationFromMapbox } from 'src/infra/maps/mapbox.service';
...
export class FoodService {
  ...
  private async clearFoodCache(
    ...
    await Promise.all([
      ...
      restaurantId
        ? this.cacheService.deleteByPattern(`food:restaurant:${restaurantId}:*`)
        : Promise.resolve(0),
      categoryId
        ? this.cacheService.deleteByPattern(`food:category:${categoryId}:*`)
        : Promise.resolve(0),
    ]);
  }
  ...
  async create(createFoodDto: CreateFoodDto): Promise<Food> {
    try {
      ...
      food.discountPercent = createFoodDto.discountPercent
        ? parseFloat(createFoodDto.discountPercent)
        : 0;
      food.status = createFoodDto.status || 'available';
      food.purchasedNumber = createFoodDto.purchasedNumber
        ? parseInt(createFoodDto.purchasedNumber, 10)
        : 0;
      food.soldCount = 0;
      food.rating = 0;
      food.imageUrls = [];
      food.tag = '';
      food.preparationTime = createFoodDto.preparationTime
        ? parseInt(createFoodDto.preparationTime, 10)
        : 0;
      ...
      if (createFoodDto.toppings && createFoodDto.toppings.length > 0) {
        const toppings = createFoodDto.toppings.map((toppingDto) => {
          ...
          topping.isAvailable =
            toppingDto.isAvailable !== undefined ? toppingDto.isAvailable : true;
      ...
      throw new BadRequestException(`Failed to create food: ${message}`);
    }
  }
  ...
  async findByRestaurantAndCategory(
    ...
    const cacheKey = buildCacheKey('food:byRestaurantAndCategory', {
      restaurantId,
      categoryId,
      page,
      pageSize,
  ...
  async findByRestaurant(
    ...
    const cacheKey = buildCacheKey('food:byRestaurant', {
      restaurantId,
      page,
      pageSize,
      lat,
  ...
  async findByCategory(
    ...
    const cacheKey = buildCacheKey('food:byCategory', {
      categoryId,
      page,
      pageSize,
      lat,
  ...
  async update(id: string, updateFoodDto: UpdateFoodDto): Promise<Food> {
    ...
    await this.clearFoodCache(
      updatedFood.id,
      updatedFood.restaurant?.id || oldRestaurantId,
      updatedFood.category?.id || oldCategoryId,
    );
    return updatedFood;
  }
  ...
  async addTopping(foodId: string, createToppingDto: CreateToppingDto): Promise<Topping> {
    ...
    topping.isAvailable =
      createToppingDto.isAvailable !== undefined ? createToppingDto.isAvailable : true;
    topping.food = food;
  ...
  async updateTopping(toppingId: string, updateToppingDto: UpdateToppingDto): Promise<Topping> {
    ...
    if (updateToppingDto.isAvailable !== undefined)
      topping.isAvailable = updateToppingDto.isAvailable;
  ...
  async getReviewsByFood(
    ...
    const averageResult = await statsQuery.select('AVG(review.rating)', 'average').getRawOne();

    const averageRating = averageResult?.average
      ? Number(parseFloat(averageResult.average).toFixed(1))
      : null;
